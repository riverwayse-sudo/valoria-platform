...
async function saveTaster({ name, role, experience, answers, results }) {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/taster_sessions`, {
      method: "POST",
      headers: {
...
const inputStyle = {
  width:"100%", boxSizing:"border-box", padding:"13px 14px", background:"rgba(255,255,255,.04)",
  border:"1px solid rgba(247,244,238,.12)", borderRadius:6, color:BRAND.PARCHMENT,
  fontFamily:BRAND.FONT_UI, fontSize:14, outline:"none",
};
...
export default function VALUIndexV4Gate({ onComplete }) {
  ...
  const canStart = name.trim() && role.trim() && experience;
  const progress = Math.round(((current + (phase === "taster" ? 0 : 0)) / TASTER_QUESTIONS.length) * 100);
  const sortedScores = useMemo(() => results ? Object.entries(results.normalisedScores).sort(([,a],[,b]) => b-a) : [], [results]);

  function start() {
    if (!canStart) return;
    setCurrent(0); setAnswers({}); setSelected(null); setPhase("taster");
  }
  ...
  async function next() {
    ...
    if (current + 1 < TASTER_QUESTIONS.length) {
      setCurrent(current + 1); setSelected(null);
      return;
    }
    const r = computeTasterResult(nextAnswers);
    setResults(r); setSaving(true);
    await saveTaster({ name:name.trim(), role:role.trim(), experience, answers:nextAnswers, results:r });
    setSaving(false); setPhase("results");
  }

  if (phase === "intro") return (
    <div style={overlayStyle}>
      <div style={cardStyle}>
        <div style={eyebrow}>VALORIA INSTITUTE · VALU INDEX v{VALU_VERSION}</div>
        <h1 style={titleStyle}>Find out where you stand.</h1>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,margin:"24px 0"}}>
          {["9 questions","~5 minutes","Free"].map(x=><div key={x} style={statStyle}>{x}</div>)}
        </div>
        <div style={fieldGrid}>
          <label style={label}>YOUR NAME<input value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" style={inputStyle}/></label>
          <label style={label}>YOUR ROLE<input value={role} onChange={e=>setRole(e.target.value)} placeholder="Current professional role" style={inputStyle}/></label>
          <label style={{...label,gridColumn:"1 / -1"}}>EXPERIENCE<select value={experience} onChange={e=>setExperience(e.target.value)} style={{...inputStyle,appearance:"auto"}}><option value="">Select your experience</option>{EXPERIENCE_BANDS.map(b=><option key={b.id} value={b.id}>{b.label} · {b.desc}</option>)}</select></label>
        </div>
  ...
  if (phase === "taster") return (
    <div style={overlayStyle}>
      <div style={{...cardStyle,maxWidth:760}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,marginBottom:18}}><div className="valu-v4-eyebrow" style={eyebrow}>{question.cluster} · {CLUSTER_NAMES[question.cluster]}</div><div style={counter}>{current+1} / {TASTER_QUESTIONS.length}</div></div>
  ...
  return (
    <div style={overlayStyle}>
      <div style={{...cardStyle,maxWidth:760}}>
        <div style={eyebrow}>VALU INDEX · TASTER COMPLETE</div>
        <h1 style={titleStyle}>{name}, here is your <em style={{color:BRAND.GOLD}}>directional read.</em></h1>
