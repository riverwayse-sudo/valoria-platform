...
export default async function handler(req, res) {
  ...
  if (String(assessment.name || '').trim().toLowerCase() !== String(taster.name || '').trim().toLowerCase() || String(assessment.role || '').trim().toLowerCase() !== String(taster.role || '').trim().toLowerCase()) {
    return json(res, 403, { error: 'Assessment identity does not match the teaser identity.' });
  }
  ...
  const patchRes = await fetch(`${SUPABASE_URL}/rest/v1/valu_assessments?${patchParams}`, {
    ...
    body: JSON.stringify({ user_id: taster.user_id, email: user.email || null }),
  });
  if (!patchRes.ok) return json(res, 502, { error: 'Could not attach the official score to the profile.' });

  return json(res, 200, { ok: true, user_id: taster.user_id, total_score: assessment.total_score });
}
