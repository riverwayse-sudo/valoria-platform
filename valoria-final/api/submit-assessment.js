...
// Server-authoritative assessment submission. The browser submits raw answers;
// the server recomputes the score against the canonical question bank.
...
const MAX_EXPERIENCE_LENGTH = 20;
...
function optionKey(option) {
  ...
  return JSON.stringify({ text: option.text ?? option.label ?? null, score: option.score ?? null });
}
function validateShuffleMap(shuffleMap) {
  ...
  for (let idx = 0; idx < QUESTIONS.length; idx += 1) {
    ...
    if (canonical.some((value, i) => value !== submitted[i])) return false;
  }
  return true;
}
function validateAnswers(answers) {
  if (!isPlainObject(answers) || Object.keys(answers).length !== QUESTIONS.length) return false;
  return QUESTIONS.every((question, idx) => Number.isInteger(answers[idx]) && answers[idx] >= 0 && answers[idx] < question.options.length);
}
...
export default async function handler(req, res) {
  ...
  if (experience != null && (typeof experience !== 'string' || experience.length > MAX_EXPERIENCE_LENGTH)) return json(res, 400, { error: 'Invalid experience band.' });
  if (!validateAnswers(answers)) return json(res, 400, { error: 'Assessment answers are incomplete or invalid.' });
  if (!Array.isArray(timings) || timings.length !== QUESTIONS.length || timings.some(value => !Number.isFinite(value) || value < 0 || value > 86_400_000)) return json(res, 400, { error: 'Invalid assessment timings.' });
  if (!validateShuffleMap(shuffleMap) || Object.keys(shuffleMap).length > QUESTIONS.length) return json(res, 400, { error: 'Invalid assessment presentation mapping.' });

  let results;
  try { results = computeResults(answers, timings, shuffleMap, QUESTIONS); }
  catch (err) { console.error('submit-assessment: scoring failed', err); return json(res, 400, { error: 'Could not score this submission.' }); }

  const fingerprint = computeFingerprint(name, role);
  const completedAt = new Date().toISOString();
  const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString();
  const row = {
    name: name.trim(), role: role.trim(), identity_hash: fingerprint,
    experience: experience || null, total_score: results.valuIndex,
  ...
  try {
    ...
    if (!insertRes.ok) {
      ...
      if (insertRes.status === 409) return json(res, 409, { error: 'An assessment already exists for this identity.' });
