...
// GitHub Actions invokes this endpoint; CRON_SECRET prevents public execution.
...
export async function GET(req) {
  ...
  if (auth !== `Bearer ${CRON_SECRET}` && cron !== CRON_SECRET) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.error('process-readiness-queue:', r.status, detail.slice(0, 500));
      return json({ error: 'Queue processing failed.' }, 502);
    }

    const processed = await r.json();
    return json({ ok: true, processed: Number(processed) || 0 });
  } catch (err) {
    console.error('process-readiness-queue:', err?.message || 'unknown');
    return json({ error: 'Queue processing failed.' }, 502);
  }
}
