...
export default async function handler(req, res) {
  ...
  const patch = {};
  if (USER_FIELDS.includes('user_id') && Object.prototype.hasOwnProperty.call(fields, 'user_id')) {
    if (fields.user_id !== verifiedUser.id) return res.status(403).json({ error: 'user_id must match the authenticated account.' });
    if (assessment.user_id && assessment.user_id !== verifiedUser.id) {
      return res.status(403).json({ error: 'Assessment already belongs to another account.' });
    }
    patch.user_id = verifiedUser.id;
  }

  if (Object.prototype.hasOwnProperty.call(fields, 'email')) {
    if (typeof fields.email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(fields.email)) {
      return res.status(400).json({ error: 'Invalid email.' });
    }
    if (fields.email.toLowerCase() !== String(verifiedUser.email || '').toLowerCase()) {
