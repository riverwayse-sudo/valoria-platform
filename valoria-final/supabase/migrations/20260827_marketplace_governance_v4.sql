...
-- The earlier repository migration for capability evidence was not present in the
-- connected database, so establish its required tables here before using them.
...
begin
  ...
  from public.professional_profiles p where p.id = p_professional_id;
  ...
  if not exists (select 1 from public.professional_capabilities c where c.professional_id = p_professional_id and c.is_active = true) then
...
begin readiness := public.evaluate_professional_readiness(p_professional_id); return coalesce((readiness->>'eligible')::boolean, false); end;
$$;
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  p record; readiness jsonb; eligible boolean; previous_status text; next_status text;
begin
  ...
  previous_status := p.listing_status; next_status := previous_status;
  ...
  if next_status <> previous_status then
    insert into public.professional_listing_events (professional_id,event_type,previous_status,new_status,reason,metadata)
    values (p_professional_id,case when next_status='listed' then 'AUTO_LISTED' else 'ELIGIBILITY_FAILED' end,previous_status,next_status,
      case when next_status='listed' then 'All automated VALU v4 marketplace eligibility gates passed.' else 'One or more automated marketplace eligibility gates are incomplete.' end,readiness);
  end if;
...
select p.id as professional_id, p.display_name as full_name, p.bio, p.location, p.languages, p.headline, p.listing_status, p.availability_status, p.visibility,
