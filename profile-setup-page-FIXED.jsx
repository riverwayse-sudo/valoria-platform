...
export default function ProfileSetupPage() {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (existing?.valu_index == null) {
        const { data: assessment } = await supabase
          ...
          .select('total_score, cluster_scores, designation')
  ...
  async function saveProgress(overrides) {
    ...
    await supabase.from('professional_profiles').upsert({
      ...
      // sync) above — only written if we actually have a value, so a profile
      // save never accidentally clears an existing score with `undefined`.
...
function ReviewScreen({ form, isCandidate, isSpeaker, isFacilitator, tags, videoLinks, handleFinish, saving, set, onChangeTracks }) {
  return (
    <div>
      ...
      {form.valu_index != null ? (
        <div style={{ background:'rgba(201,168,76,.05)', border:`1px solid ${GLINE}`, padding:'20px', marginBottom:'24px' }}>
          <div style={{ fontSize:'9px', fontWeight:700, letterSpacing:'.18em', color:'rgba(201,168,76,.45)', marginBottom:'10px' }}>VALU INDEX ASSESSMENT</div>
          <div style={{ display:'flex', alignItems:'baseline', gap:'10px', marginBottom:'8px' }}>
            <span style={{ fontSize:'32px', fontWeight:700, color:GOLD }}>{form.valu_index}</span>
            <span style={{ fontSize:'13px', color:DIM }}>/ 100</span>
          </div>
          {form.designation && (
            <div style={{ fontSize:'11px', fontWeight:700, color:GOLD, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:'10px' }}>{form.designation.replace(/_/g,' ')}</div>
          )}
          <p style={{ fontSize:'13px', fontWeight:300, color:DIM, lineHeight:1.7, margin:0 }}>
            {form.valu_index >= 35
              ? 'This score qualifies your profile for marketplace listing, pending review.'
              : 'This score is below the 35-point marketplace listing threshold. Your profile is saved and searchable results won\u2019t include it, but nothing else is blocked.'}
          </p>
        </div>
      ) : (
        <div style={{ background:'rgba(201,168,76,.05)', border:`1px solid ${GLINE}`, padding:'20px', marginBottom:'24px' }}>
